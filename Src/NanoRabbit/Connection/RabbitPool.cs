...
public class RabbitPool : IRabbitPool
{
    ...
    public void NanoPublish<T>(string connectionName, string producerName, T message)
    {
        try
        {
            ...
            {
                channel.ExchangeDeclare(producerConfig.ExchangeName, producerConfig.Type,
                    durable: producerConfig.Durable, autoDelete: producerConfig.AutoDelete,
                    arguments: producerConfig.Arguments);
            ...
            _logger?.LogInformation("Message published by {ProducerName} to {ConnectionName}", producerName,
                connectionName);
        }
        catch (Exception e)
        {
    ...
    public void NanoConsume<T>(string connectionName, string consumerName, Action<T> messageHandler)
    {
        try
        {
            ...
            {
                _logger?.LogInformation("Received messages from {ConnectionName} - {ConsumerConfigQueueName}",
                    connectionName, consumerConfig.QueueName);
        ...
        string toProducerName)
    {
        try
        {
            ...
            {
                _logger?.LogInformation("Received messages from {ConnectionName} - {ConsumerConfigQueueName}",
                    fromConnectionName, consumerConfig.QueueName);
                var body = Encoding.UTF8.GetString(ea.Body.ToArray());
                var message = JsonConvert.DeserializeObject<T>(body);
                if (message != null)
                {
